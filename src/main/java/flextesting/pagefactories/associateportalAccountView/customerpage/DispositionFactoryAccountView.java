...
public class DispositionFactoryAccountView extends TestBase {
...
//	private By locator_btn_Next_grayedOut_window_SelectAccountForDisposition = By.xpath("//div[@class='x-form-check-wrap']//label[contains(text(),'Auto')]//preceding-sibling::input[1]//following::button[text()='Next'][1]//ancestor::table[contains(@class,'x-item-disabled')][1]");
	...
	static Logger log = Logger.getLogger(DispositionFactoryAccountView.class);
...
//		btn_SubmitAllDispositions.click();
	...
	public boolean tryToClickDispositionBtn(boolean accountLevel) throws InterruptedException {
		...
		log.debug("Can not click " + level + " disposition btn. Pressing escape key 1 time and try again");
		pressEscapeKeyMultipleTimes(getDriver(), 1);
		if (dispositionBtnClicked(accountLevel)) return true;
		
		//if failed again, press [Cancel] then [Yes] btn if they exist
		log.debug("Still can not click " + level + " disposition btn. Clicking [Cancel] then [Yes] button 1 time and try again");
		...
		log.debug(level + " disposition btn is unclickable. Give up");
		return false;
	}
	...
	private boolean dispositionBtnClicked(boolean accountLevel) {
		...
		if (!btn.isDisplayed()) {
			log.debug(level + " disposition btn is not visible");
			return false;
		}
		...
		try {
			...
			log.debug(level + " disposition btn isDisplayed=true, but clicking it cause WebDriverException");
			return false;
		}
		//if not yet returned, meaning it's clicked successfully
		log.debug(level + " disposition btn is clicked");
		return true;
	}
