package flextesting.pagefactories.associateportalAccountView;
...
//		openInANewWindow(driver, "//div[@class='x-combo-list-item']//h3[contains(text(),'Reference Number')]");
	...
	public void searchAndLoadNonReadOnlyUserLogon() throws Exception {
		...
		while( customerLocked || accountLocked) {
			...
			if (accountLocked) {
				log.debug("A window shows up indicating the account is locked");
				...
				log.debug("Skip Verification clicked");
				log.debug("current window will be closed before switching, so get the current window and window pool now and pass them to switchWindow() later"); 
				...
				log.debug("Top right corner X button (close overall view) clicked");
				log.debug("Current tab already closed before switching, so: Switching => load another userlogon...");
				switchWindow(getDriver(), false, currentWindow, windowPool);
			} else {
				log.debug("A window shows up indicating the customer is locked");
				log.debug("No need to get current window and window pool because current window will not be closed before switching");
				log.debug("Now, during window/tab switching: close current tab => switching => load another userlogon...");
	...
	private void searchAndLoadUserLogon(String userLogon) throws Exception {
		log.debug("Try userLogon: " + userLogon);
